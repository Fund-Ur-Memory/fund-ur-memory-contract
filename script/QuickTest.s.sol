...
contract QuickTest is Script {
    ...
    function run() external view {
        console.log("=== Quick Test Cipher Contract ===");
        console.log("Testing Contract Address:", CIPHER_VAULT_ADDRESS);
        ...
        try cipherVault.getContractStats() returns (uint256 totalVaults, uint256 contractBalance) {
        ...
        try cipherVault.checkInterval() returns (uint256 interval) {
        ...
        try cipherVault.lastCheckTimestamp() returns (uint256 lastCheck) {
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            try cipherVault.getCurrentPrice(tokens[i]) returns (uint256 price) {
            ...
            try cipherVault.getPriceFeedInfo(tokens[i]) returns (
                address priceFeed,
                uint256 heartbeat,
                uint8 decimals
            ) {
        ...
        try cipherVault.supportedTokens(address(0)) returns (bool supported) {
        ...
        try cipherVault.owner() returns (address contractOwner) {
            console.log("Contract Owner:", contractOwner);
        } catch {
            console.log("Error getting contract owner");
        }

        try cipherVault.paused() returns (bool isPaused) {
...
contract TestContractInteraction is Script {
    ...
    function run() external {
        ...
        try cipherVault.getOwnerVaults(userAddress) returns (uint256[] memory userVaultIds) {
            ...
            if (userVaultIds.length > 0) {
                ...
                if (userVaultIds.length > 0) {
                    ...
                    try cipherVault.getVault(firstVaultId) returns (CipherVault.Vault memory vault) {
                        ...
                        // Check conditions
                        try cipherVault.checkConditions(firstVaultId) returns (bool conditionsMet) {
